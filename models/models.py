    ...
    def show_results(self):
        for current in self:
            ...
            if current.type == '1':
                ...
                l = []
                v = []
                # Itération sur les IDs des tâches spécifiées dans l'enregistrement actuel (current)
                for tt in current.task_ids.ids:
                    ...
                    if this_p.kit_id:
                        ...
                        l.append(this_p.name)
                # Vérification des IDs des tâches pour construire des requêtes SQL et récupérer les IDs de tâches correspondantes
                if len(v) > 0:
                    tasks = self.env['project.task.work'].search([
                        ('project_id', '=', current.project_id.id),
                        ('kit_id', 'in', v),
                ...
                # Vérification des IDs des tâches pour construire des requêtes SQL et récupérer les IDs des tâches liées
                if len(l) > 0:
                    ll1 = self.env['base.task.merge.automatic.wizard.project.task.work_rel'].search([
                        ('base_task_merge_automatic_wizard_id', '=', current.id),
                    ])
                    if not ll1:
                        raise exceptions.UserError(_("Merci de sauvegarder le document avant!"))
                        ...
                        res_cpt = [nn.project_task_work_id.id for nn in ll1]
                    # Intersection des IDs des tâches récupérées à partir des deux requêtes SQL
                    pp = set(tasks.ids).intersection(res_cpt)
                else:
                    res_cpt = tasks.ids
            ...
            for kk in res_cpt:
                if kk:
                    ...
                    sequence_w = 0
                    res = self.env['project.task.work'].search([
                        ('task_id', '=', s2.task_id.id),
                        ('sequence', '!=', False)
                    ], order='sequence desc', limit=1)
                    sequence_w = res.sequence if res else 0

                    for jj in current.line_ids:
                        if jj.secteur > jj.secteur_to:
                            raise exceptions.UserError(
                                _("Le secteur de départ doit être plus petit que le secteur de fin!"))
